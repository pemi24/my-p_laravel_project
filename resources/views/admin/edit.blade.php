<!DOCTYPE html>
<html lang="en" dir="ltr">
  <head>
    <meta charset="utf-8">
   <title>user page</title>
   <link rel="stylesheet" href="navbar.css">
   <link rel="stylesheet" href="/user.css">
   <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.2/css/all.min.css" />
   <meta name="viewport" content="width=device-width, initial-scale=1.0">
   <!-- Include SweetAlert library -->
   

  </head>
  <body>
  @extends('home.homelayout')
  @section('content')
  <div >
  <div class="row">
  <div class="col-md-15" style="margin-top: 80px;">
 
<div class="ard" >
    <section>
    <div class="container">    
      <div class="contactInfo">
      	<div>
      		<h2>Contact Info</h2>
      		<ul class="info">
      		  <li>
      			<span><img src="/location.png"></span>
      			<span>Logbaba</span>
      		  </li>
      		  <li>
      			<span><img src="/mail.png"></span>
      			<span>miclanjunior@gmail.com</span>
      		  </li>
      		  <li>
      			<span><img src="/call.png"></span>
      			<span>+237676772832</span>
      		  </li>
      		</ul>
      	</div>
      	<ul class="sci">
      		<li><a href="#"><img src="/1.png"></a></li>
      		<li><a href="#"><img src="/2.png"></a></li>
      		<li><a href="#"><img src="/3.png"></a></li>
      		<li><a href="#"><img src="/5.png"></a></li>
      	</ul>
      </div>
      <div class="contactForm">
      	<h2>Send a Message</h2>
        <form action="{{  url('admin/' .$admin->id) }}" method="post" onsubmit ="showMessage()">

        {!! csrf_field() !!}
        @method("PATCH")
        <input type="hidden" name="id" id="id" value="{{$admin->id}}" id="id" />
      	<div class="formBox">
      		<div class="inputBox w50">
      			<input type="text" name="firstName" value="{{$admin->firstName}}"  required>
      			<span>First Name</span>
      		</div>
      		<div class="inputBox w50">
      			<input type="text" name="lastName" value="{{$admin->lastName}}"  required>
      			<span>Last Name</span>
      		</div>
      		<div class="inputBox w50">
      			<input type="text" name="email" value="{{$admin->email}}" required>
      			<span>Email Address</span>
      		</div>
      		<div class="inputBox w50">
      			<input type="number" name="phoneNumber" value="{{$admin->phoneNumber}}" required>
      			<span>Mobile Number</span>
      		</div>
      		<div class="inputBox w100">
      			<textarea name="message" required>{{$admin->message}}</textarea> 
      			<span>Write your message here...</span>
      		</div>
      		<div class="inputBox w100">
      			<input type="submit" value="Save" class="btn btn-success" >
      		</div>
      	</div>
      </form>
      </div>
    </div>
    </section>
  </div>
</div>
</div>
</div>
</div>

<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
<script>
  function showMessage() {
    Swal.fire({
      icon: 'success',
      title: 'Success',
      text: 'Your message has been updated successfully',
      showConfirmButton: true,
      timer: 10000, // Set the duration in milliseconds (5 seconds)
      timerProgressBar: true,
      didOpen: () => {
        Swal.showLoading();
      },
      willClose: () => {
        // Code to execute when the alert is closed
        // This can be left empty if no action is needed
      }
    });
  }
</script>


    @endsection
  </body>
</html>